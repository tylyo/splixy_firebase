...
exports.usersSavewalletshareLink = functions.database.ref("/users/{uid}/joinreq/{sl}").onWrite(async (snap, context) => {
  ...
  await adminDB.ref(COLLECTION_WALLETS).orderByChild("sl").equalTo(sharedLinkID).limitToFirst(1).get().then(async (snap) => {
    ...
    const walletJoinReqPath = [COLLECTION_JOINREQUEST, uid, sharedLinkID].join(PATH_SEPARATOR);
    updates[walletJoinReqPath + "/gid"] = walletId;
    updates[walletJoinReqPath + "/t"] = wallet.name;
    updates[walletJoinReqPath + "/pvp"] = wallet.rules.pvp;
    updates[walletJoinReqPath + "/dr"] = currentTM();
    console.log("updates: " + JSON.stringify(updates));
...
exports.walletUpdateQuotas = functions.database.ref("/wallets/{walletId}/updQuota/").onWrite((change, context) => {
  ...
  adminDB.ref(COLLECTION_TRANSACTIONS).child(walletId).once('value', (snap, _) => {
    ...
    for (const k in updatesMember) {
      const memberValues = updatesMember[k]
...
function calculate(currTrx) {
  ...
  const paid = currTrx.a * currTrx.qt;
  var parts = 0
  for (const key in currTrx.debs) {
    parts += currTrx.debs[key];
  }
  ...
  if (trxType == 'item') {
    ...
    var quotaAmount = round2Dec(paid / parts);
    if (quotaAmount !== round2Dec(currTrx.q.a)) console.warn(trxId, "QUOTA DIVERSA QUELLA REGISTRATA", currTrx.qt, paid, parts, quotaAmount, currTrx.q.a);
    for (const key in currTrx.debs) {
      
      const part = currTrx.debs[[key]];
      const debt = round2Dec(part * quotaAmount);
      quotaCheck += debt;
    ...
    const delta = round2Dec(paid) - round2Dec(quotaCheck)
...
function trxUpdateToWallet(walletId, currTrx, nextTrx) {
  ...
  for (const key in nextDebitors) {
    console.log("updateWalletQuotaLast LOOP", key, currCalc[key].s, nextCalc[key].s, currCalc[key].s - nextCalc[key].s);
    const paid = nextCalc[key].p - currCalc[key].p
    const debt = nextCalc[key].s - currCalc[key].s
...
function walletQuotasCalculate(walletId, trxList) {
  ...
  for (const trxId in trxList) {
    ...
    for (const key in nextCalc) {
      if (key === "0xDwg3qHwj") {
        console.log("------------- START -- ", cc, trxId);
        console.log(nextTrx);
        console.log("walletQuotasCalculate LOOP", cc)
      }
      // console.log("walletQuotasCalculate LOOP", key)
      var prevCalc = new WalletMember(key, 0, 0, 0);
      if (updatesMember.hasOwnProperty(key)) {
        prevCalc = updatesMember[key];
      }
      if (key === "0xDwg3qHwj") {
        console.log("walletQuotasCalculate LOOP", cc, "prev", key, prevCalc)
        console.log("walletQuotasCalculate LOOP", cc, "next", key, nextCalc[key])
      }
...
function getWalletQuotaUpdate(currTrx, nextTrx) {
  ...
  for (const key in nextTrx.debs) {
    ...
    walletMember.q = nextQuotaDebit - currQuotaDebit;
